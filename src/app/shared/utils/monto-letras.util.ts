...
export type MonedaCode = 'PYG' | 'USD' | 'BRL';
...
const MONEDAS: Record<MonedaCode, MonedaDef> = {
 ...
 */
export function montoEnLetras(monto: number, moneda: MonedaCode | string = 'PYG'): string {
  const code = ((moneda || 'PYG') as string).toUpperCase() as MonedaCode;
  const def = MONEDAS[code];
  if (!def) throw new Error(`Moneda no soportada: ${moneda}`);
...
export function montoEnLetrasConPrefijo(monto: number, moneda: MonedaCode | string = 'PYG'): string {
