...
@Component({
  ...
  templateUrl: './list-operacion-financiera-categorias.component.html',
  styleUrls: ['./list-operacion-financiera-categorias.component.scss'],
  standalone: true,
  imports: [
...
export class ListOperacionFinancieraCategoriasComponent implements OnInit {
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      this.categorias = await firstValueFrom(this.repositoryService.getOperacionFinancieraCategorias());
    } catch (error) {
      console.error('Error loading op. fin. categorias:', error);
  ...
  async saveInlineForm(): Promise<void> {
    ...
    try {
      ...
      if (this.editingId) {
        await firstValueFrom(this.repositoryService.updateOperacionFinancieraCategoria(this.editingId, data));
        this.snackBar.open('Categoria actualizada', 'Cerrar', { duration: 3000 });
      } else {
        await firstValueFrom(this.repositoryService.createOperacionFinancieraCategoria(data));
        this.snackBar.open('Categoria creada', 'Cerrar', { duration: 3000 });
      }
      this.cancelInlineForm();
      this.loadData();
    } catch (error) {
      console.error('Error saving op fin categoria:', error);
  ...
  async desactivar(c: any): Promise<void> {
    const ref = this.dialog.open(ConfirmationDialogComponent, {
      data: { title: 'Desactivar Categoria', message: `¿Desactivar "${c.nombre}"?` },
    });
    const ok = await firstValueFrom(ref.afterClosed());
    if (!ok) return;
    try {
      await firstValueFrom(this.repositoryService.deleteOperacionFinancieraCategoria(c.id));
      this.snackBar.open('Categoria desactivada', 'Cerrar', { duration: 3000 });
      this.loadData();
    } catch (error) {
      console.error('Error desactivando op fin categoria:', error);
