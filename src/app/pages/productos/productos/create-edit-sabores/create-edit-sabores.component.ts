...
export class CreateEditSaboresComponent implements OnInit {
  ...
  loadPresentacionSabores(): void {
    this.loading = true;
    this.repositoryService.getPresentacionSabores(this.data.presentacion.id).subscribe({
  ...
  addSabor(): void {
    ...
    this.repositoryService.createPresentacionSabor(newPresentacionSabor).subscribe({
      next: () => {
        this.loadPresentacionSabores();
        this.cancelAddSabor();
  ...
  createSabor(): void {
    ...
    this.repositoryService.createSabor(newSabor).subscribe({
  ...
  updateReceta(): void {
    ...
    this.repositoryService.updatePresentacionSabor(this.currentPresentacionSabor.id, {
      recetaId: recetaId
    }).subscribe({
      next: () => {
        this.loadPresentacionSabores();
  ...
  toggleSaborStatus(presentacionSabor: PresentacionSabor): void {
    ...
    this.repositoryService.updatePresentacionSabor(presentacionSabor.id, {
      activo: newStatus
    }).subscribe({
      next: () => {
        // Update locally
  ...
  deleteSabor(presentacionSabor: PresentacionSabor): void {
    if (confirm(`¿Está seguro de eliminar este sabor?`)) {
      this.loading = true;
      this.repositoryService.deletePresentacionSabor(presentacionSabor.id).subscribe({
        next: () => {
          this.loadPresentacionSabores();
