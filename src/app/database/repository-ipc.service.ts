...
interface ElectronAPI {
 ...
 // PrecioDelivery operations
  ...
  editCajaMayorMovimientos: (ediciones: any[]) => Promise<any>;
...
/**
 * Implementación del repositorio usando Electron IPC.
 *
 * F2: por DI, los componentes inyectan `RepositoryService` (abstract) y
 * la factory en `app.module.ts` decide entre esta clase (modo standalone
 * o server) y `RepositoryHttpService` (modo cliente).
 *
 * `providedIn: 'root'` se mantiene para que Angular pueda crear instancias
 * cuando un provider lo requiera. La factory en app.module también puede
 * inyectarla via `deps: [RepositoryIpcService]` si necesita ambas para
 * decidir.
 */
@Injectable({
  providedIn: 'root'
...
export class RepositoryIpcService extends RepositoryService {
  ...
  editCajaMayorMovimientos(ediciones: any[]): Observable<any> {
    return from(this.api.editCajaMayorMovimientos(ediciones));
  }
