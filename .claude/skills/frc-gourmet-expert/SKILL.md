...
| **Buscador global** (command palette Ctrl+Espacio, registro de menús, handler `buscar-global`) | [domains/buscador-global.md](domains/buscador-global.md) |
...
22. **Toda pantalla navegable nueva debe registrarse en el buscador global.** No solo los menús del sidenav: también sub-pantallas abiertas con `openTab(...)` desde otros componentes y **diálogos-destino** (lanzadores de config sin contexto). Agregar la entrada en `MENU_ENTRIES` (`src/app/services/menu-registry.ts`): para tabs `{ id, label, keywords, icon, group, esConfig, permiso, title, component, tabId, data }`; para diálogos sumar `openMode: 'dialog'` + `dialogConfig`. El registro tiene dos partes: las del sidenav (generadas) y un bloque de **extras curados** (destinos fuera del sidenav). NO auto-incluir diálogos contextuales (edit/create/confirm/selector/detalle/...). Si no se registra, la pantalla funciona pero NO aparece en el buscador (Ctrl+Espacio). Detalles → [domains/buscador-global.md](domains/buscador-global.md).
