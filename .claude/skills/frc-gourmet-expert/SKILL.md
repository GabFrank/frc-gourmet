...
- **POS ruidosos ocultos por defecto (PR #175)** — `TIPOS_BANCARIOS_RUIDOSOS` + opt `excluirRuidosos` (reemplazó `excludePos`). La consolidada oculta POS por defecto; toggle "Ver POS/ocultos" en `caja-mayor-detalle`. La vista de cuenta individual muestra todo.
- **Anular VALE desde Caja Mayor (PR #178)** — `anular-caja-mayor-movimiento` con `valeId` ya no bloquea: delega la anulación (contra-mov `AJUSTE_POSITIVO` + saldo + `Vale` a ANULADO) en un click.
- **OCR (PR #178)** — umbral de auto-vínculo de producto bajó a `vecesUsado >= 1` (auto-vincula desde la 2da importación). "Importar con IA" es mat-menu (archivo / QR) en list-compras y list-factura-imports. `crear-presentacion-inline-dialog` (opción centinela en el select de presentaciones). Resumen WhatsApp: "Total de ventas" agrupado por moneda + total en Gs.
