...
- **Release y deploy:** `semantic-release` + GitHub Actions. Push a `master` → canal `stable`, `release/beta` → `beta`, `develop` → `alpha`. ⚠️ **En la práctica hoy sólo se usa `alpha`** (confirmado por Gabriel el 2026-08-24): `beta` no existe como canal de este producto y `stable` es historia vieja. Todo lo que se mergea a `develop` es lo que corre. No hablar de «promover a beta» — no hay adónde. Cada push dispara `release.yml` → tag + GitHub Release + instaladores Windows NSIS / Linux AppImage. La app trae **auto-update** (`electron-updater`). La branch de releases es **`master`** (NO `main` — `origin/main` está obsoleta). Detalles → [workflows/release-y-deploy.md](workflows/release-y-deploy.md).
