...
22. **Todo menú nuevo del sidenav debe registrarse en el buscador global.** Al agregar un `openXxxTab()` (`app.component.ts`) + su item en `app.component.html`, agregar SIEMPRE la entrada correspondiente en `MENU_ENTRIES` (`src/app/services/menu-registry.ts`) con `{ id, label, keywords, icon, group, esConfig, permiso, title, component, tabId, data }` EXACTOS del `openTab(...)`. Si no se registra, el menú funciona pero NO aparece en el buscador (Ctrl+Espacio). Es parte del checklist de agregar una pantalla. Detalles → [domains/buscador-global.md](domains/buscador-global.md).
