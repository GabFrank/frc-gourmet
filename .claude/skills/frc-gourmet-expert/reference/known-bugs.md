...
- **`src/app/shared/components/cierre-caja-dialog/` es CÓDIGO MUERTO** — sin usos. El cierre real de caja lo hace `create-caja-dialog` en modo `'conteo'`. No editarlo esperando cambiar el cierre.
- **Ingresar el cierre de una caja PdV a la Caja Mayor: existe.** El retiro del cierre se **auto-genera** al cerrar (`generarRetiroDelCierre`, `electron/handlers/retiro-cierre.util.ts`, estado FLOTANTE). Acceso: *Detalle de Caja Mayor → "Registrar Ingreso" → "Retiro de Caja de Venta"* (lista retiros FLOTANTE, incluidos los de cierre); o *Dashboard Caja Mayor → "Retiros"*. El botón "Generar retiro del cierre" en `list-cajas` lo **backfillea** (idempotente) para cierres viejos.
- **Faltantes falsos en el cierre — RESUELTO (PR #172).** `computeResumenCaja` filtra `PagoDetalle.activo=true`; antes sumaba líneas anuladas por `anularCobroParcial` e inflaba el "esperado". Si ves un faltante inexplicable, verificá primero que el filtro `activo` siga en `resumen-caja.utils.ts`, `getVentasTotalByCaja`, `dashboard-ventas.handler.ts` y `getVentasByDateRange`.
- **Todo en los tickets térmicos se imprimía como "Gs." — RESUELTO.** `monedaSimboloAscii` leía `moneda.codigo`/`moneda.nombre` (campos inexistentes). La entity `Moneda` usa `countryCode`/`simbolo`/`denominacion`. Ahora mapea por `countryCode`. → [conventions/pitfalls-typeorm-electron.md](../conventions/pitfalls-typeorm-electron.md).
