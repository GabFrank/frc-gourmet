...
`electron/handlers/permissions.handler.ts` — el array `SEED_PERMISOS` define **100 permisos** pre-cargados al startup (`seedPermissions()`, idempotente por `codigo`). Agregar un permiso = añadirlo al array; al siguiente arranque se inserta y `syncAdminPermissions()` se lo asigna al rol ADMINISTRADOR.
