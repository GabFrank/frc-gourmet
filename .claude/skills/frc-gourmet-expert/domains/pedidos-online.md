...
**Config UI:** los flags `permiteMesa` / `requiereLanMesa` / `rangoLanMesa` se editan desde *Config Tienda Online* (sección "Pedidos en mesa (QR)"). `requiereLanMesa` se apaga en desarrollo. **Todas las fases (F1–F5 + F3b + F2b) están implementadas** en la rama `claude/pedidos-mesa-qr`.
