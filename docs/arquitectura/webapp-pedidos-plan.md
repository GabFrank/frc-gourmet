...
| Fase 1 — Menú publicable | 🟩 Backend + toggle UI | Entidad+migración flags online, handler `get-menu-online` (público `menu.get`), create/update-producto persisten flags. **Toggles "Disponible online" + "Pausado (86)" en el form de producto.** Pendiente: pantalla "Carta Online" dedicada + reactivar imágenes. |
| **Tests** | 🟩 e2e (35) | `scripts/test-pedidos-online-e2e.ts` (`npm run test:pedidos-online`): 35 asserts del flujo real (menú, OTP, **refresh token**, pedido pickup/delivery, **config de tienda: apertura/tipos/auto-accept**, bandeja admin) contra SQLite+Fastify. 35/35 ✅. |
| **Imágenes** | 🟩 storefront | Endpoint público `/pub/producto-image/:file` (server) + `AppImgPipe` en el storefront → las fotos de producto se ven en la carta. (Alta de imagen desde el form de producto: pipeline existente.) |
| Fase 2 — Config tienda + auth cliente | ✅ Completa | Auth OTP+password, **refresh token** (`CustomerRefreshToken` + `auth.refresh`/`auth.logout`, auto-refresh en el storefront). **`TiendaOnlineConfig` + pantalla admin** (apertura/horarios, tipos de pedido, prep-time, mínimo, aceptación automática, branding); el storefront la respeta (abierto/cerrado, tipos, branding). |
