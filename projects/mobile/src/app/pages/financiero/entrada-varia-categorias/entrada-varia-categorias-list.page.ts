...
/** Lista de Categorías de entrada varia (Financiero). Permiso CAJA_MAYOR_OPERAR. */
@Component({
  selector: 'app-entrada-varia-categorias-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './entrada-varia-categorias-list.page.html',
})
export class EntradaVariaCategoriasListPage implements OnInit {
  ...
  cargar(): void {
    this.loading = true;
    this.error = null;
    this.repo.getEntradaVariaCategorias().subscribe({
      next: (data) => {
  ...
  async eliminar(c: CategoriaVM): Promise<void> {
    ...
    if (!ok) return;
    this.repo.deleteEntradaVariaCategoria(c.id).subscribe({
      next: () => {
