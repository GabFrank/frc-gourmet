...
/** Lista de Categorías de operación financiera (Financiero). Permiso CAJA_MAYOR_OPERAR. */
@Component({
  selector: 'app-operacion-financiera-categorias-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './operacion-financiera-categorias-list.page.html',
})
export class OperacionFinancieraCategoriasListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.perm.codigos$.subscribe(() => (this.canEdit = this.perm.has('CAJA_MAYOR_OPERAR')));
    this.cargar();
  }
  ...
  cargar(): void {
    this.loading = true;
    this.error = null;
    this.repo.getOperacionFinancieraCategorias().subscribe({
      next: (data) => {
        this.todos = (data || []) as CategoriaVM[];
        this.aplicarFiltro();
        this.loading = false;
      },
      error: () => {
  ...
  aplicarFiltro(): void {
    ...
    this.items = q ? this.todos.filter((c) => (c.nombre || '').toLowerCase().includes(q)) : [...this.todos];
  }
  ...
  async eliminar(c: CategoriaVM): Promise<void> {
    ...
    if (!ok) return;
    this.repo.deleteOperacionFinancieraCategoria(c.id).subscribe({
      next: () => {
