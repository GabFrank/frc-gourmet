...
/**
 * Pagar una cuota de Cuenta por Pagar desde la Caja Mayor (efectivo) o desde una
 * Cuenta bancaria. Para banco se ofrecen solo cuentas de la MISMA moneda de la
 * cuota (evita cotización). Requiere COMPRAS_GESTIONAR. Devuelve `true` si se pagó.
 */
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      ...
      <form [formGroup]="form" class="col">
        ...
        <p class="efectivo" *ngIf="!esBanco">Se paga en <strong>{{ efectivoLabel }}</strong> desde la caja mayor.</p>
...
export class PagarCppDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    Promise.all([
      ...
      .then(([cajas, formas, cuentas]: [any[], any[], any[]]) => {
        ...
        // Solo cuentas de la MISMA moneda de la cuota (sin cotización en mobile).
  ...
  async pagar(): Promise<void> {
    ...
    const payload = this.esBanco
      ? { cuotaId: this.data.cuotaId, monto: v.monto, fuente: 'CUENTA_BANCARIA', cuentaBancariaId: v.cuentaBancariaId, observacion: obs }
      : { cuotaId: this.data.cuotaId, monto: v.monto, fuente: 'CAJA_MAYOR', cajaMayorId: v.cajaMayorId, monedaId: this.data.monedaId, formaPagoId: this.formaPagoId, observacion: obs };
    try {
