...
/** Lista de Categorías de gasto (Financiero). Permiso CAJA_MAYOR_OPERAR. */
@Component({
  selector: 'app-gasto-categorias-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './gasto-categorias-list.page.html',
})
export class GastoCategoriasListPage implements OnInit {
  ...
  cargar(): void {
    this.loading = true;
    this.error = null;
    this.repo.getGastoCategorias().subscribe({
      next: (data) => {
  ...
  async eliminar(c: CategoriaVM): Promise<void> {
    ...
    if (!ok) return;
    this.repo.deleteGastoCategoria(c.id).subscribe({
      next: () => {
