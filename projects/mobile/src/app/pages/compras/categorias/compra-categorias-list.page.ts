...
/** Lista de Categorías de compra. Permiso COMPRAS_GESTIONAR. */
@Component({
  selector: 'app-compra-categorias-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './compra-categorias-list.page.html',
})
export class CompraCategoriasListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.perm.codigos$.subscribe(() => (this.canEdit = this.perm.has('COMPRAS_GESTIONAR')));
    this.cargar();
  }
  ...
  cargar(): void {
    this.loading = true;
    this.error = null;
    this.repo.getCompraCategorias().subscribe({
      next: (data) => {
  ...
  async eliminar(c: CategoriaVM): Promise<void> {
    ...
    if (!ok) return;
    this.repo.deleteCompraCategoria(c.id).subscribe({
      next: () => {
